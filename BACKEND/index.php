   ...
   <section>
      ...
      <div class="flex">
          ...
          if(!empty($dados['SendLogin'])){
              var_dump($dados);
              $query_email = "SELECT id, email, senha_email
              FROM email
              WHERE email =:email
              LIMIT 1";
              $result_email = $conn->prepare($query_email);
              $result_email->bindParam('email', $dados['email'], POD::PARAM_STR);
             $result_email->execute();
 $row_email = $result_email->fetch(POD::FETCH_ASSOC);
 var_dump($row_email);
          ...
          <form method ="POST" action= "index.php">
            ...
            <label>senha</label>
            <input type="password" name="senha_email" placeholder="digite sua senha"><br><br>
            <input type="submit" value="login" name="login" class="btn">
         </form>

         <form method="post" action="/PHP/processamento-cliente.php">
            <h3>cadastre-se</h3>
